                                        ...
                                        <div class="card border-primary border pricing-card-body">
                                            ...
                                            <div class="wrapper">
                                                @if ($subscription)
                                                    ...
                                                    @elseif ($paymentConfiguration && $paymentConfiguration->payment_method == 'ccavenue' && $paymentConfiguration->status == 1 && $subscription->package_type == 0)
                                                        <form action="{{ route('subscriptions.store') }}" method="POST">
                                                            @csrf
                                                            <input type="hidden" name="payment_method" value="ccavenue">
                                                            <input type="hidden" name="package_id" value="addon">
                                                            <input type="hidden" name="type" value="addon">
                                                            <input type="hidden" name="subscription_id" value="{{ $subscription->id }}">
                                                            <input type="hidden" name="feature_id" value="{{ $addon->feature_id }}">
                                                            <input type="hidden" name="addon_id" value="{{ $addon->id }}">
                                                            <button class="btn btn-outline-success w-100">{{ __('add') }}</button>
                                                        </form>
                                                    @else
                                                        <button data-id="{{ $addon->id }}" data-type="{{ $subscription->package_type }}" class="btn btn-outline-success add-addon btn-block">{{ __('add') }}</button>
                                                    @endif
