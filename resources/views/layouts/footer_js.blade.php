...
<script>
    ...
    let date_format = '{{ data_get($schoolSettings ?? null, "date_format") ?? data_get($schoolSetting ?? null, "date_format") ?? data_get($systemSettings ?? null, "date_format") ?? "d-m-Y" }}'.replace('Y', 'YYYY').replace('m', 'MM').replace('d', 'DD');

    let date_time_format = '{{ data_get($schoolSettings ?? null, "date_format") ?? data_get($schoolSetting ?? null, "date_format") ?? data_get($systemSettings ?? null, "date_format") ?? "d-m-Y" }} {{ data_get($schoolSettings ?? null, "time_format") ?? data_get($schoolSetting ?? null, "time_format") ?? data_get($systemSettings ?? null, "time_format") ?? "h:i A" }}'.replace('Y', 'YYYY').replace('m', 'MM').replace('d', 'DD').replace('h', 'hh').replace('H', 'HH').replace('i', 'mm').replace('a', 'a').replace('A', 'A');

    let time_format = '{{ data_get($schoolSettings ?? null, "time_format") ?? data_get($schoolSetting ?? null, "time_format") ?? data_get($systemSettings ?? null, "time_format") ?? "h:i A" }}'.replace('h', 'hh').replace('H', 'HH').replace('i', 'mm').replace('a', 'a').replace('A', 'A');
    ...
    $(document).ready(function() {
        $("#menu-search, #menu-search-mini").on("keyup", function() {
            ...
            $(".sidebar .nav-link").each(function() {
                var link = $(this);
                // Target menu-title span if available, else use link text
                var target = link.find('.menu-title').length > 0 ? link.find('.menu-title') : link;
            ...
            if (value === "") {
                $(".sidebar .nav-item").show().removeClass('search-result-item');
                $(".sidebar .collapse").not('.active-parent').removeClass('show');
                $(".sidebar .nav-item.active").parents('.collapse').addClass('show');
                return;
            }

            // Hide everything initially
            $(".sidebar .nav-item").hide().removeClass('search-result-item');
            ...
            $(".sidebar .nav-link").each(function() {
                ...
                if (text.indexOf(value) > -1) {
                    ...
                    // Show and highlight this item
                    item.show().addClass('search-result-item');
                    
                    // Show all parent items and expand intermediate collapses
                    item.parents(".nav-item").show();
                    item.parents(".collapse").addClass('show');
                    
                    // If this is a main category, show its sub-items too
                    var subMenu = item.find(".sub-menu");
                    if (subMenu.length > 0) {
                        subMenu.find(".nav-item").show();
                    ...
                    var regex = new RegExp("(" + value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + ")", "gi");
                    var highlightedHTML = originalText.replace(regex, '<span class="search-highlight">$1</span>');
                    target.html(highlightedHTML);
                }
            });
            ...
            if ($firstMatch) {
                ...
                if (sidebar) {
                    $(sidebar).stop().animate({
                        scrollTop: $(sidebar).scrollTop() + $firstMatch.position().top - 100
                    }, 200);
                }
            }
        });
    ...
    $('.navbar-toggler').click(function (e) { 
        setTimeout(() => {
            if ($('body').hasClass('sidebar-icon-only')) {
                $('.sidebar-search').addClass('d-none');
            } else {
                $('.sidebar-search').removeClass('d-none');
            }
        }, 200);
    });
