...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderExamData(data) {
        ...
        if (data.success && data.exams && data.exams.length > 0) {
            ...
            // Update Overview Summary if elements exist
            const overviewExamBody = document.getElementById('overview_exam_body');
            if (overviewExamBody && allExams.length > 0) {
                const latestExam = allExams[0];
                let summaryHtml = '';
                
                if (latestExam.exam_type === 'Online Exam') {
                    summaryHtml = `
                        <h6 class="mb-2 text-primary font-weight-bold">${latestExam.name || '{{ __("Online Exam") }}'}</h6>
                        <div class="d-flex justify-content-between mb-1 small">
                            <span class="text-muted">{{ __('Subject') }}</span>
                            <span>${latestExam.subject_name}</span>
                        </div>
                        <div class="d-flex justify-content-between mb-1 small">
                            <span class="text-muted">{{ __('Total Marks') }}</span>
                            <span>${latestExam.total_marks}</span>
                        </div>
                        <div class="d-flex justify-content-between mb-2 small">
                            <span class="text-muted">{{ __('Obtained') }}</span>
                            <span class="text-success font-weight-bold">${latestExam.total_obtained_marks}</span>
                        </div>
                        <span class="badge badge-primary">{{ __('Online') }}</span>
                        <span class="badge ${latestExam.status === 'Pass' ? 'badge-success' : 'badge-danger'}">${latestExam.status}</span>
                    `;
                } else {
                    const summary = latestExam.summary || {};
                    const subjects = latestExam.subjects || [];
                    
                    // Add subject list if available
                    let subjectsHtml = '';
                    if (subjects.length > 0) {
                        subjectsHtml = `
                            <div class="mb-3 mt-2" style="max-height: 120px; overflow-y: auto; scrollbar-width: thin;">
                                <table class="table table-sm table-borderless mb-0" style="font-size: 11px;">
                                    <tbody>
                                        ${subjects.map(s => `
                                            <tr class="border-bottom">
                                                <td class="pl-0 py-1 text-muted">${s.name}</td>
                                                <td class="pr-0 py-1 text-right font-weight-bold">${s.obtained_marks}/${s.max_marks}</td>
                                            </tr>
                                        `).join('')}
                                    </tbody>
                                </table>
                    ...
                    summaryHtml = `
                        <h6 class="mb-2 text-success font-weight-bold">${latestExam.name || '{{ __("Examination Result") }}'}</h6>
                        ${subjectsHtml}
                        <hr class="my-2">
                        <div class="d-flex justify-content-between mb-1 small">
                            <span class="text-muted">{{ __('Total Marks') }}</span>
                            <span>${summary.max_marks || 0}</span>
                        </div>
                        <div class="d-flex justify-content-between mb-1 small">
                            <span class="text-muted">{{ __('Obtained') }}</span>
                            <span class="text-success font-weight-bold">${summary.obtained_marks || 0}</span>
                        </div>
                        <div class="d-flex justify-content-between mb-2 small">
                            <span class="text-muted">{{ __('Percentage') }}</span>
                            <span class="font-weight-bold">${Math.round(summary.percentage || 0)}%</span>
                        </div>
                        <div class="mt-2 text-center">
                            <span class="badge ${getResultBadgeClass(summary.result)}">${summary.result || '{{ __("N/A") }}'}</span>
                            ${summary.rank ? `<span class="badge badge-info ml-1">{{ __("Rank") }}: ${summary.rank}</span>` : ''}
                        </div>
                    `;
                }
                overviewExamBody.innerHTML = summaryHtml;
            }
