    ...
    public function up(): void
    {
        ...
        if (Schema::hasTable('schools')) {
            Schema::table('schools', static function (Blueprint $table) {
                if (Schema::hasColumn('schools', 'admin_id')) {
                    try {
                        DB::statement('ALTER TABLE schools DROP FOREIGN KEY schools_admin_id_foreign');
                    } catch (\Exception $e) {
                    }
                    try {
                        DB::statement('ALTER TABLE schools ADD CONSTRAINT schools_admin_id_foreign FOREIGN KEY (admin_id) REFERENCES users(id) ON DELETE CASCADE');
                    } catch (\Exception $e) {
                    }
                }
            });
        ...
        Schema::table('payment_configurations', static function (Blueprint $table) {
            if (!Schema::hasColumn('payment_configurations', 'bank_name')) {
                $table->string('bank_name')->nullable(true)->after('webhook_secret_key');
            }
            if (!Schema::hasColumn('payment_configurations', 'account_name')) {
                $table->string('account_name')->nullable(true)->after('bank_name');
            }
            if (!Schema::hasColumn('payment_configurations', 'account_no')) {
        ...
        Schema::table('students', static function (Blueprint $table) {
            if (!Schema::hasColumn('students', 'application_type')) {
        ...
        if (Schema::hasTable('features')) {
            try {
                Schema::table('features', static function (Blueprint $table) {
                    $table->tinyInteger('status')->default(1)->after('is_default');
                });
            } catch (\Exception $e) {}

            try {
                Schema::table('features', static function (Blueprint $table) {
                    $table->integer('required_vps')->after('is_default')->default(0);
                });
            } catch (\Exception $e) {}
        }
        ...
        if (Schema::hasTable('schools')) {
            Schema::table('schools', static function (Blueprint $table) {
                ...
                if (!Schema::hasColumn('schools', 'database_name')) {
                    $table->string('database_name')->nullable(true);
                }
        ...
        if (Schema::hasTable('staffs')) {
            Schema::table('staffs', static function (Blueprint $table) {
                if (!Schema::hasColumn('staffs', 'joining_date')) {
