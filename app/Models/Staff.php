    ...
    public function scopeOwner($query)
    {
        if (Auth::user()) {
            if (Auth::user()->hasRole('Super Admin')) {
                if (request()->has('school_id') && !empty(request('school_id')) && request('school_id') != 'all' && request('school_id') != 'undefined') {
                    return $query->where('school_id', request('school_id'));
                }
                if (request()->has('session_year_id') && request('session_year_id') != 'all') {
