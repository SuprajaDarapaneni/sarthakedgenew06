...
class SubscriptionService
{
    ...
    {
        $subscriptionBill = $this->subscriptionBill->builder()->where(function($q) {
            $q->whereHas('transaction', function ($q) {
                $q->whereNot('payment_status', "succeed");
            })->orDoesntHave('transaction');
        })->where('school_id', Auth::user()->school_id)->whereNot('amount', 0)->first();
    ...
    // CCAvenue Payment
    public function ccavenue_payment($subscriptionBill_id = null, $package_id = null, $type = null, $subscription_id = null, $isCurrentPlan = null)
    {
        try {
            $amount = 0;
            $order_id = 'ORD' . time();
            ...
            if ($subscriptionBill_id && $subscriptionBill_id != -1) {
                $subscriptionBill = $this->subscriptionBill->findById($subscriptionBill_id);
                $amount = $subscriptionBill->amount;
                $order_id = 'BILL' . $subscriptionBill_id . 'T' . time();
            }

            if ($package_id && $package_id != -1) {
                $package = $this->package->findById($package_id);
                $amount = $package->charges;
                $order_id = 'PKG' . $package_id . 'T' . time();
            }
            ...
            $paymentTransactionData = app(\App\Repositories\PaymentTransaction\PaymentTransactionInterface::class)->create([
                'user_id' => $user->id,
                'amount' => $amount,
            ...
            $metadata = [
                ...
                'subscriptionBill_id' => $subscriptionBill_id,
                'payment_transaction_id' => $paymentTransactionData->id,
                'school_id' => $schoolId,
                'merchant_param1' => $paymentTransactionData->id,
                'merchant_param2' => $schoolId,
                'merchant_param3' => 'subscription',
                'redirect_url' => route('ccavenue.callback'),
                'cancel_url' => route('ccavenue.callback'),
            ];

            // Store metadata in signature
            $paymentTransactionData->update(['payment_signature' => json_encode($metadata)]);

            $paymentService = new PaymentService();
            $ccavenue = $paymentService->create('ccavenue');
            $paymentData = $ccavenue->prepare(array_merge($metadata, ['amount' => $amount, 'order_id' => $order_id]));

            return view('payment.ccavenue_redirect', [
                'url' => $paymentData['url'],
            ...
            Log::error('CCAvenue Payment Error: ' . $e->getMessage());
            return redirect()->back()->with('error', trans('server_not_responding'));
        }
    }
    ...
    public function prepaid_addon_payment($addonSubscriptionId)
    {
        try {
            ...
            // CCAvenue Payment
            if(strtolower($paymentConfiguration->payment_method) == 'ccavenue') {
                $paymentService = new PaymentService();
                $ccavenue = $paymentService->create('ccavenue');

                $params = [
                    'amount' => $addonSubscription->price,
                    'order_id' => 'ADDON' . $addonSubscriptionId . 'T' . time(),
                    'currency' => 'INR',
                    'redirect_url' => url('ccavenue/callback'),
                    'cancel_url' => url('ccavenue/callback'),
                    'merchant_param1' => 'addon',
                    'merchant_param2' => $addonSubscriptionId,
                ];

                $paymentData = $ccavenue->prepare($params);
