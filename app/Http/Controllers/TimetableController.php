...
class TimetableController extends Controller
{
    ...
    public function index()
    {
        ...
        $classes = $this->classSection->builder()->with('class', 'section', 'medium', 'class.stream')->get()->pluck('full_name', 'id');
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            if ($timetable_start_time <= $start_time && $timetable_end_time >= $end_time) {
                $this->timetable->updateOrCreate(['id' => $id,], array_merge($request->all(), ['school_id' => Auth::user()->school_id]));
    ...
    public function show(Request $request)
    {
        ...
        $sql = $this->classSection->builder()->where('id', $classSectionId)->with([
            'class:id,name,stream_id',
            'class.stream',
            'section:id,name',
            'medium:id,name',
            'timetable' => function ($query) {
                $query->with('subject:id,name,type', 'subject_teacher.teacher:id,first_name,last_name');
            }
        ]);
    ...
    {
        ResponseService::noFeatureThenRedirect('Timetable Management');
        ResponseService::noPermissionThenSendJson(['timetable-create']);
        $request->validate([
            'from_class_section_id' => 'required|numeric',
            'to_class_section_id' => 'required|numeric',
        ]);
        try {
            DB::beginTransaction();
            $fromTimetables = $this->timetable->builder()->where('class_section_id', $request->from_class_section_id)->get();

            // Delete existing timetable for 'to' class section
            $this->timetable->builder()->where('class_section_id', $request->to_class_section_id)->delete();

            foreach ($fromTimetables as $timetable) {
                $newTimetable = $timetable->replicate();
                $newTimetable->class_section_id = $request->to_class_section_id;
                $newTimetable->save();
            }
            DB::commit();
            ResponseService::successResponse('Timetable copied successfully');
        } catch (Throwable $e) {
            DB::rollBack();
            ResponseService::logErrorResponse($e);
            ResponseService::errorResponse();
