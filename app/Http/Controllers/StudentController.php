...
class StudentController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'admission_no' => ['required', new uniqueForSchool('students', 'admission_no')],
        ...
        try {
            ...
            if ($classSection->capacity > 0 && $classSection->students->count() >= $classSection->capacity) {
                ResponseService::errorResponse("Section is full. Available seats: 0");
            }
    ...
    public function update($id, Request $request)
    {
        ...
        try {
            ...
            // Check Section Capacity if class section is changed
            $student = $this->student->builder()->where('user_id', $id)->first();
            if ($student->class_section_id != $request->class_section_id) {
                $classSection = $this->classSection->findById($request->class_section_id, ['*'], [
                    'students' => function ($q) {
                        $q->whereHas('user', function ($u) {
                            $u->where('status', 1);
                ...
                if ($classSection->capacity > 0 && $classSection->students->count() >= $classSection->capacity) {
                    ResponseService::errorResponse("Target section is full. Available seats: 0");
                }
            }
    ...
    public function resetPasswordUpdate(Request $request)
    {
        ...
        try {
            DB::beginTransaction();
            // If a custom password is provided (e.g. via modal), validate and use it.
            if ($request->filled('password')) {
                ...
                // Fallback: existing behaviour – use DOB (ddmmyyyy) as password
                $dob = date('dmY', strtotime($request->dob));
                $newPassword = Hash::make($dob);
            }

            $this->user->update($request->id, ['password' => $newPassword, 'reset_request' => 0]);
            DB::commit();
