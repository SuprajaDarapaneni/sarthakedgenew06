...
class PayrollController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            foreach ($user_ids as $key => $user_id) {
                $staff = $this->staff->builder()->with('user')->findOrFail($user_id);
                $school_id = $staff->user->school_id;
                
                $paid_leaves = $request->paid_leave[$user_id] ?? 0;
                ...
                $expense = $this->expense->updateOrCreate(
                ...
                $this->sessionYearsTrackingsService->storeSessionYearsTracking('App\Models\Expense', $expense->id, Auth::user()->id, $sessionYearInterface->id, $school_id, null);
    ...
    public function slip($id = null)
    {
        ...
        try {
            // Salary
            $salary = $this->expense->builder()->with('staff.user:id,first_name,last_name,school_id', 'staff_payroll.payroll_setting')->where('id', $id)->first();
            if (!$salary) {
                return redirect()->back()->with('error', trans('no_data_found'));
            }

            $schoolSetting = $this->cache->getSchoolSettings('*', $salary->school_id);
            $systemSettings = $this->cache->getSystemSettings();

            if (!$salary->school_id || empty($schoolSetting['school_name'])) {
                $schoolSetting['school_name'] = $systemSettings['app_name'] ?? $systemSettings['tag_line'] ?? 'SarthakEdge';
                $schoolSetting['school_address'] = $systemSettings['address'] ?? '';
                $schoolSetting['favicon'] = $systemSettings['favicon'] ?? null;
                $schoolSetting['horizontal_logo'] = $systemSettings['horizontal_logo'] ?? null;
            }
