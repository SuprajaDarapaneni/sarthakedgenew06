...
class ClassSchoolController extends Controller
{
    ...
    public function index()
    {
        ...
        $classes = $this->class->builder()->orderBy('id', 'DESC')->with('stream', 'medium', 'sections', 'class_sections.students', 'next_class')->get();
        ...
        foreach ($classes as $class) {
            $total_sections_count += $class->sections->count();
            foreach ($class->class_sections as $section) {
                $total_students += $section->students->count();
            }
        }
        $active_classes = $total_classes; // Simulating active for now
        $teachers = \App\Models\User::role('Teacher')->get();

        return response(view('class.index', compact('classes', 'sections', 'mediums', 'subjects', 'semesters', 'shifts', 'streams', 'total_classes', 'total_students', 'total_sections_count', 'active_classes', 'teachers')));
    }
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'next_class_id' => 'nullable|numeric',
        ]);
        ...
        try {
            ...
            if (!empty($request->stream_id) && $request->stream_id[0] != null) {
                foreach ($request->stream_id as $stream) {
                    ...
                    if (!empty($teacherId)) {
                        $insertedSections = $this->classSection->builder()->where('class_id', $class->id)->get();
                        foreach ($insertedSections as $sec) {
            ...
            if ($teacherId) {
                \App\Models\User::find($teacherId)->givePermissionTo(['class-teacher', 'exam-upload-marks', 'exam-result', 'attendance-list']);
            }
    ...
    public function edit($id)
    {
        ...
        $streams = $this->stream->all();
        $mediums = $this->medium->builder()->orderBy('id', 'ASC')->get();
        $classes = $this->class->builder()->where('id', '!=', $id)->orderBy('id', 'DESC')->get(); // Exclude current class
        return response(view('class.edit', compact('class', 'subjects', 'sections', 'class_section', 'id', 'semesters', 'shifts', 'streams', 'mediums', 'classes')));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            // Section Update
            $selectedSections = $request->section_id ?? [];
            // Delete sections that are NOT in the selected list
            $this->classSection->builder()->where('class_id', $id)->whereNotIn('section_id', $selectedSections)->delete();
            ...
            if (!empty($selectedSections)) {
                ...
                foreach ($selectedSections as $section_id) {
                    $class_section[] = array(
                        'class_id' => $id,
                        'section_id' => $section_id,
                        'medium_id' => $request->medium_id ?? $class->medium_id,
                        'deleted_at' => null
                    );
                }
                $this->classSection->upsert($class_section, ['class_id', 'section_id'], ['deleted_at', 'medium_id']);
            }
    ...
    public function show(Request $request)
    {
        ...
        $semesters = $this->semester->all();
        $sql = $this->class->builder()->with('stream', 'sections', 'medium', 'stream:id,name', 'shift:id,name', 'class_teachers.teacher', 'class_sections.students', 'all_subjects', 'next_class')
        ...
        foreach ($res as $row) {
            ...
            if (empty($showDeleted)) {
                $tempRow['section_names'] = $row->sections->pluck('name')->implode(', ');
            } else {
                $tempRow['section_names'] = $row->section->pluck('name')->implode(', ');
            }

            // Student Count
            $tempRow['student_count'] = 0;
            foreach ($row->class_sections as $section) {
                $tempRow['student_count'] += $section->students->count();
            }

            // Subject Count
            $tempRow['subject_count'] = $row->all_subjects->count();

            // Class Teacher
            $teachers = [];
            foreach ($row->class_teachers as $ct) {
                if ($ct->teacher) {
                    $teachers[] = $ct->teacher->full_name;
                }
            }
            $tempRow['class_teacher'] = count($teachers) > 0 ? implode(', ', $teachers) : __('No Teacher');
            $tempRow['status'] = 'active'; // Classes in the main list are active
            $tempRow['next_class_id'] = $row->next_class_id;
            $tempRow['next_class_name'] = $row->next_class->full_name ?? '-';


            $tempRow['semesters'] = $semesters;
            $tempRow['operate_edit_url'] = route('class.edit', $row->id);
            $tempRow['operate_delete_url'] = route('class.destroy', $row->id);
