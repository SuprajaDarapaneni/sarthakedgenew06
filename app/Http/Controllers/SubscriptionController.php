...
class SubscriptionController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            if (strtolower($request->payment_method) == 'stripe') {
                return $this->subscriptionService->stripe_payment(null, $request->package_id, $request->type, null, null);
            } else if (strtolower($request->payment_method) == 'paystack') {
                return $this->subscriptionService->paystack_payment(null, $request->package_id, $request->type, null, null);
            } else if (strtolower($request->payment_method) == 'flutterwave') {
                return $this->subscriptionService->flutterwave_payment(null, $request->package_id, $request->type, null, null);
            } else if (strtolower($request->payment_method) == 'ccavenue') {
                return $this->subscriptionService->ccavenue_payment($request->subscription_id, $request->package_id, $request->type, null, null);
            }
        } catch (\Throwable $th) {
            \Log::error('Subscription Store Error: ' . $th->getMessage());
    ...
    public function prepaid_plan($package_id, $type = null, $isCurrentPlan = null)
    {
        ...
        try {
            ...
            } else if (strtolower($paymentConfiguration->payment_method) == 'ccavenue') {
                return $this->subscriptionService->ccavenue_payment(null, $package_id, $type, null, $isCurrentPlan);
            }
    ...
    public function subscription_report()
    {
        ...
        $expiring_soon = $this->subscription->builder()
            ->whereDate('end_date', '>=', $today)
            ->whereDate('end_date', '<=', $sevenDaysLater)
            ->count();

        // Total revenue from paid subscription bills
        $total_revenue = \App\Models\SubscriptionBill::whereHas('transaction', function ($q) {
            $q->where('payment_status', 'succeed');
        })->sum('amount');
    ...
    public function pay_prepaid_upcoming_plan($package_id, $type, $subscription_id)
    {
        ...
        try {
            ...
            if ($paymentConfiguration->payment_method == 'Stripe') {
                ...
                return $this->subscriptionService->ccavenue_payment(null, $package_id, $type, $subscription_id);
            }
