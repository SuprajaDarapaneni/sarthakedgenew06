...
class ExamController extends Controller
{
    ...
    public function index()
    {
        ...
        ResponseService::noAnyPermissionThenRedirect(['exam-list', 'exam-create', 'exam-upload-marks', 'exam-result', 'class-teacher']);
    ...
    {
        ResponseService::noFeatureThenRedirect('Exam Management');
        ResponseService::noAnyPermissionThenSendJson(['exam-list', 'exam-create', 'exam-upload-marks', 'exam-result', 'class-teacher']);
        $request->validate([
            'name' => 'required',
            'session_year_id' => 'required',
            'class_id' => 'required|array',
            'class_id.*' => 'exists:classes,id',
            'exam_type' => 'nullable|in:1,2',
        ]);

        try {
            DB::beginTransaction();
            $sessionYear = $this->cache->getDefaultSessionYear();
            $exam_type = $request->exam_type ?? 1; // 1: Offline, 2: Online
    ...
    {
        Log::info("Exam List - Show method started for ID: " . $id);
        ResponseService::noFeatureThenRedirect('Exam Management');
        ResponseService::noAnyPermissionThenSendJson(['exam-list', 'exam-create', 'exam-upload-marks', 'exam-result', 'class-teacher']);
        ...
        Log::info("Exam List - Request Params: Offset=$offset, Limit=$limit, Search=$search, Year=" . request('session_year_id'));

        try {
            Log::info("Exam List - Getting Settings");
            $schoolSettings = $this->cache->getSchoolSettings();
            $dateFormat = (!empty($schoolSettings) && isset($schoolSettings['date_format'])) ? $schoolSettings['date_format'] : 'd-m-Y';

            Log::info("Exam List - Building SQL");
            ...
            })->when(request()->filled('exam_type'), function ($query) {
            ...
            Log::info("Exam List - Calculating Count");
            $total = $sql->count();
            Log::info("Exam List - Total found: " . $total);
            ...
            Log::info("Exam List - Fetching Results");
            $sql->orderBy($sort, $order)->skip($offset)->take($limit);
            $res = $sql->get();
            Log::info("Exam List - Records fetched: " . count($res));

            $bulkData = array();
            $bulkData['total'] = $total;
            if ($user->school_id == 9 || $user->id == 1) {
                $bulkData['diag_sql'] = $sql->toSql();
                $bulkData['diag_bindings'] = $sql->getBindings();
            }
            
            Log::info("Exam List - Fetching Class Sections");
            $classSections = $this->classSection->builder()->get()->toArray();
            Log::info("Exam List - Class Sections count: " . count($classSections));
            ...
            if ($row->class) {
                ...
                if ($row->has_timetable) {
                    foreach ($row->timetable as $timetable) {
                        ...
                        $marks = collect([]);
                        if ($class_section_id) {
                            $marks = $timetable->exam_marks->filter(function($mark) use($class_section_id) {
                                return ($mark->user && $mark->user->student && $mark->user->student->class_section_id == $class_section_id);
                            });
                        }
            ...
            $tempRow['created_at'] = date($dateFormat, strtotime($row->created_at));
            $tempRow['updated_at'] = date($dateFormat, strtotime($row->updated_at));
                    $rows[] = $tempRow;
                } catch (\Throwable $e) {
                    \Log::error("Error processing exam row ID: " . ($row->id ?? 'unknown') . " - " . $e->getMessage());
                    continue;
                }
            }
            $bulkData['rows'] = $rows;
            Log::info("Exam List - Returning Success Response");
            return response()->json($bulkData);
        } catch (\Throwable $e) {
            Log::error("Exam List - GLOBAL CRASH: " . $e->getMessage());
            ResponseService::logErrorResponse($e, "Exam Controller -> Show method");
    ...
    {
        ResponseService::noFeatureThenRedirect('Exam Management');
        ResponseService::noAnyPermissionThenRedirect(['exam-upload-marks', 'exam-list', 'exam-result', 'class-teacher']);
    ...
    public function marksList(Request $request)
    {
        ...
        try {
            ...
            // Exam status check removed - allow marks entry regardless of exam date/status

            $sessionYearId = $exam->session_year_id; // Get Students Data on the basis of Exam's Session Year
            $students = $this->users->builder()->role('Student')->whereIn('id', $studentIds)->with([
                ...
                ->whereHas('student', function ($q) use ($sessionYearId) {
    ...
    public function downloadSampleFile(Request $request)
    {
        ...
        try {
            ...
            // IF Exam status is 0 that is exam not started then show error message
            if ($exam->exam_status == 0) {
                ResponseService::errorRedirectResponse(null, 'Exam has not started yet');
            }

            $sessionYearId = $exam->session_year_id;
            $students = $this->users->builder()->role('Student')->whereIn('id', $studentIds)->with([
                'exam_marks' => function ($query) use ($timetable) {
                    $query->where('exam_timetable_id', $timetable->id);
                }
    ...
    public function getExamByClassId($class_section_id)
    {
        ...
        try {
            $sessionYear = $this->cache->getDefaultSessionYear();
            $class_id = $this->classSection->builder()->where('id', $class_section_id)->pluck('class_id')->first();
            $exams = $this->exam->builder()->where('class_id', $class_id)->where('publish', 0)->get();
