...
class SchoolController extends Controller
{
    ...
    public function index()
    {
        ...
        // For school code generation
        $latestSchool = $this->schoolsRepository->builder()->latest()->first();
        try {
        ...
        $school_code = date('Y') . (($latestSchool->id ?? 0) + 1);
        ...
        $schoolsList = $this->schoolsRepository->builder()->with('user', 'extra_school_details.form_field')
            ->with([
            ...
            ->when($package_id, function ($query) use ($package_id, $today_date) {
                $query->whereHas('subscription', function ($q) use ($package_id, $today_date) {
        ...
        return view('schools.index', compact('packages', 'baseUrlWithoutScheme', 'school_code', 'prefix', 'demoSchool', 'extraFields', 'email_verified', 'schoolsList'));
    }
    ...
    {
        ResponseService::noPermissionThenRedirect('schools-edit');

        $school = $this->schoolsRepository->builder()
            ->with('user', 'extra_school_details.form_field', 'subscription.package')
            ->where('id', $id)
            ->firstOrFail();
        ...
        $currentSubscription = $school->subscription()->whereDate('start_date', '<=', Carbon::now())->whereDate('end_date', '>=', Carbon::now())->latest()->first();
        $currentPackageId = $currentSubscription ? $currentSubscription->package_id : null;

        return view('schools.edit', compact('school', 'packages', 'extraFields', 'baseUrlWithoutScheme', 'currentPackageId'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            // Safety check: Ensure the school record exists in the tenant database to avoid FK errors in school_settings
            if (!DB::connection('school')->table('schools')->where('id', $request->edit_id)->exists()) {
                $centralSchool = School::on('mysql')->find($request->edit_id);
                if ($centralSchool) {
                    // Disable FK checks to break circular dependency between schools <-> users
                    DB::connection('school')->statement('SET FOREIGN_KEY_CHECKS=0');

                    // Sync Admin User first (to satisfy schools.admin_id FK)
                    if (!DB::connection('school')->table('users')->where('id', $centralSchool->admin_id)->exists()) {
                        $centralAdmin = User::on('mysql')->find($centralSchool->admin_id);
                        if ($centralAdmin) {
                            $adminData = (array)DB::connection('mysql')->table('users')->where('id', $centralSchool->admin_id)->first();
                            $userColumns = Schema::connection('school')->getColumnListing('users');
                            $filteredAdminData = array_intersect_key($adminData, array_flip($userColumns));
                            DB::connection('school')->table('users')->insert($filteredAdminData);
                        }
                    }

                    // Sync School record
                    $schoolDataToSync = (array)DB::connection('mysql')->table('schools')->where('id', $request->edit_id)->first();
                    $schoolColumns = Schema::connection('school')->getColumnListing('schools');
                    $filteredSchoolData = array_intersect_key($schoolDataToSync, array_flip($schoolColumns));
                    DB::connection('school')->table('schools')->insert($filteredSchoolData);
            ...
            if ($request->assign_package) {
                // Expire any currently active subscriptions before assigning a new one
                $today = Carbon::now()->format('Y-m-d');
                $yesterday = Carbon::now()->subDay()->format('Y-m-d');
                \App\Models\Subscription::where('school_id', $request->edit_id)
