...
class SystemSettingsController extends Controller {
    ...
    public function paymentUpdate(Request $request) {
        ...
        $request->validate([
            ...
            'gateway.Ccavenue.status' => 'required|boolean',
            'gateway.Ccavenue.merchant_id' => 'required_if:gateway.Ccavenue.status,1',
            'gateway.Ccavenue.secret_key' => 'required_if:gateway.Ccavenue.status,1',
            'gateway.Ccavenue.api_key' => 'required_if:gateway.Ccavenue.status,1',
        ], [
            ...
            'gateway.Ccavenue.merchant_id.required_if' => trans('The CCAvenue Merchant ID is required when CCAvenue is enabled'),
            'gateway.Ccavenue.secret_key.required_if' => trans('The CCAvenue Working Key is required when CCAvenue is enabled'),
            'gateway.Ccavenue.api_key.required_if' => trans('The CCAvenue Access Code is required when CCAvenue is enabled'),
        ]);
        // $request->validate([
