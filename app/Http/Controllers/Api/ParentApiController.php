...
//            $studentData = $this->student->findById($request->child_id, ['id', 'user_id', 'class_section_id'], ['class_section']);
    ...
    public function payCompulsoryFees(Request $request)
    {
        ...
        try {
            ...
            if (strtolower($request->payment_method) == "ccavenue") {
                ...
                $ccavenueDetails = [
                    "payment_link" => route('ccavenue.webview', ['transaction_id' => $paymentTransactionData->id, 'school_id' => $schoolId]),
    ...
    {
        \Log::info("payOptionalFees endpoint HIT by user " . Auth::id(), $request->all());
        $validator = Validator::make($request->all(), [
            'child_id' => 'required',
            'fees_id' => 'required',
