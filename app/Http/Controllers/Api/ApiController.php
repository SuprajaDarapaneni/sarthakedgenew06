    ...
    public function getPaymentTransactions(Request $request)
    {
        ...
        try {
            ...
            $paymentTransactions = $paymentTransactions->map(function ($data) use ($schoolSettings) {
                ...
                if (strtolower($data->payment_status) == "pending") {
                    try {
                        if ($data->order_id) {
                                            ...
                                            if (isset($paymentIntent['metadata']['type']) && $paymentIntent['metadata']['type'] == 'fees') {
                                                ...
                                                if ($fees) {
                                                    ...
                                                    $feesPaidData = [
                                                        ...
                                                        'is_used_installment' => !empty($data->installment_details)
                                                    ];
                                                    ...
                                                    $metaObj = (object)$paymentIntent['metadata'];
                                                    if ($metaObj->fees_type == "compulsory") {
                                                        $webhookController->processCompulsoryFees($data, $feesPaidResult, $metaObj);
                                                    } else if ($metaObj->fees_type == "optional") {
                                                        $webhookController->processOptionalFees($data, $feesPaidResult, $metaObj);
                                                    }
                                                }
                                            } else {
