...
class StaffApiController extends Controller
{
    ...
    {
        try {
            $sql = $this->user->findById(Auth::user()->id, ['*'], ['staff', 'class_teacher.class_section.class', 'class_teacher.class_section.section', 'class_teacher.class_section.medium']);

            if ($sql) {
                $class_names = [];
                $class_sections_data = [];
                if ($sql->relationLoaded('class_teacher')) {
                    foreach ($sql->class_teacher as $ct) {
                        if ($ct->class_section) {
                            $class_names[] = $ct->class_section->class->name . ' - ' . $ct->class_section->section->name . ' (' . $ct->class_section->medium->name . ')';
                            
                            $class_sections_data[] = [
                                'id' => $ct->class_section->id,
                                'class_id' => $ct->class_section->class_id,
                                'section_id' => $ct->class_section->section_id,
                                'class_name' => $ct->class_section->class->name,
                                'section_name' => $ct->class_section->section->name,
                                'medium_name' => $ct->class_section->medium->name,
                                'full_name' => $ct->class_section->class->name . ' - ' . $ct->class_section->section->name . ' (' . $ct->class_section->medium->name . ')'
                            ];
                        }
                    }
                }
                $sql->class_teacher_of = implode(", ", $class_names);
                $sql->class_teacher_sections = $class_sections_data;
            }
