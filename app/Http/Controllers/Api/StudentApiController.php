...
class StudentApiController extends Controller
{
    ...
    public function getGuardianDetails(Request $request)
    {
        try {
            ...
            if ($guardian) {
                // Ensure flat 'address' field is available for app
                $guardian->address = $guardian->current_address ?? $guardian->permanent_address ?? '';
            }
            $data = array(
                'guardian' => (!empty($guardian)) ? $guardian : (object) []
            );
