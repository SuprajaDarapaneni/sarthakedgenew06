...
class TeacherApiController extends Controller
{
    ...
    #[NoReturn] public function login(Request $request)
    {
        ...
        if (
            ...
            if (Auth::user()->hasRole('Teacher')) {
                $user = $auth->load(['teacher', 'teacher.staffSalary.payrollSetting', 'class_teacher.class_section.class', 'class_teacher.class_section.section', 'class_teacher.class_section.medium']);
            } else {
                $user = $auth->load(['staff', 'staff.staffSalary.payrollSetting', 'class_teacher.class_section.class', 'class_teacher.class_section.section', 'class_teacher.class_section.medium']);
            }
    ...
    public function getExamList(Request $request)
    {
        ...
        try {
            ...
            $sql = $this->exam->builder()->with('session_year', 'class', 'exam_class_sections')->select('id', 'name', 'description', 'class_id', 'start_date', 'end_date', 'session_year_id', 'publish')
                ->with([
            ...
            $subjectTeacherIdsAll = SubjectTeacher::where('teacher_id', $teacher_id)->get();
            $classTeacherSectionsAll = ClassTeacher::where('teacher_id', $teacher_id)->pluck('class_section_id')->toArray();
            ...
            foreach ($exam_data_db as $data) {
                // Use model attribute for consistent status calculation
                $exam_status = (string)$data->exam_status;

                // Determine teacher involvement for this specific exam
                $exam_class_section_ids = $data->exam_class_sections->pluck('class_section_id')->toArray();
                
                $is_class_teacher = false;
                $active_subject_ids = [];

                if ($request->class_section_id) {
                    $is_class_teacher = in_array($request->class_section_id, $classTeacherSectionsAll);
                    $active_subject_ids = $subjectTeacherIdsAll->where('class_section_id', $request->class_section_id)->pluck('subject_id')->toArray();
                } else {
                    $is_class_teacher = count(array_intersect($classTeacherSectionsAll, $exam_class_section_ids)) > 0;
                    $active_subject_ids = $subjectTeacherIdsAll->whereIn('class_section_id', $exam_class_section_ids)->pluck('subject_id')->unique()->toArray();
                }
                ...
                if (count($data->timetable)) {
                    foreach ($data->timetable as $key => $timetable) {
                        ...
                        } else if (in_array($timetable->class_subject->subject_id, $active_subject_ids)) {
                            $subject = [
                                'id' => $timetable->class_subject->subject->id,
                                'name' => $timetable->class_subject->subject->name,
                                'type' => $timetable->class_subject->subject->type,
