...
class FeesController extends Controller
{
    ...
    public function feesConfigIndex()
    {
        ...
        $ccavenueData = $this->paymentConfigurations->all()->filter(function($item) {
            return strtolower($item->payment_method) === 'ccavenue';
        })->first();
        return view('fees.fees_config', compact('settings', 'domain', 'stripeData', 'ccavenueData'));
    }
    ...
    public function feesConfigUpdate(Request $request)
    {
        ResponseService::noFeatureThenRedirect('Fees Management');
        ResponseService::noPermissionThenRedirect('fees-config');
        $request->validate(['stripe_status' => 'required', 'stripe_publishable_key' => 'required_if:stripe_status,1|nullable', 'stripe_secret_key' => 'required_if:stripe_status,1|nullable', 'stripe_webhook_secret' => 'required_if:stripe_status,1|nullable', 'stripe_webhook_url' => 'required_if:stripe_status,1|nullable', 'ccavenue_status' => 'required', 'ccavenue_merchant_id' => 'required_if:ccavenue_status,1|nullable', 'ccavenue_working_key' => 'required_if:ccavenue_status,1|nullable', 'ccavenue_access_code' => 'required_if:ccavenue_status,1|nullable', 'currency_code' => 'required|max:10', 'currency_symbol' => 'required|max:5',]);
        try {
            ...
            $this->paymentConfigurations->updateOrCreate(['payment_method' => 'Ccavenue'], ['merchant_id' => $request->ccavenue_merchant_id, 'secret_key' => $request->ccavenue_working_key, 'api_key' => $request->ccavenue_access_code, 'status' => $request->ccavenue_status, 'currency_code' => $request->currency_code]);
    ...
    public function feesTransactionsLogsList(Request $request)
    {
        ...
        if (!empty($request->payment_gateway) && $request->payment_gateway !== 'cash') {
            $sql->where('payment_gateway', $request->payment_gateway);
        }
        ...
        if (!empty($request->session_year_id)) {
            $sessionYearRecord = $this->sessionYear->findById($request->session_year_id);
            if ($sessionYearRecord) {
                $year = date('Y', strtotime($sessionYearRecord->getRawOriginal('start_date') ?? $sessionYearRecord->start_date));
                $sql->whereYear('created_at', (int) $year);
            }
        }
    ...
    public function payCompulsoryFeesStore(Request $request)
    {
        ...
        $request->validate([
            ...
            'installment_fees' => 'array|required_if:installment_mode,1',
        ], [
