...
class ReportsController extends Controller
{
    ...
    public function student_view_reports($id, $session_year_id)
    {
        ...
        // Calculate fee summary for Overview
        $feeSummary = [
            'total' => $studentFees->sum('amount'),
            'paid' => $studentFees->filter(fn($f) => $f->is_fully_paid)->sum('amount'),
            'due' => $studentFees->filter(fn($f) => !$f->is_fully_paid)->sum('amount'), // simplified
            'status' => 'Pending'
        ];
        ...
        return view('reports.student.student-view-reports', compact('student', 'studentFees', 'sessionYears', 'sessionYear', 'session_year_id', 'feeSummary'));
    }
