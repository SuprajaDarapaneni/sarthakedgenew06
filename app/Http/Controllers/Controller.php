...
class Controller extends BaseController
{
    ...
    public function index()
    {
        ...
        if ($this->isSchoolWebsiteRequest()) {
            ...
            // Change view name to home while deploying live
            return view('home1', compact('features', 'packages', 'settings', 'faqs', 'guidances', 'languages', 'schoolSettings', 'featureSections', 'about_us_lists', 'counter', 'trail_package', 'extraFields', 'demoSchoolUrl', 'allSchools', 'isDemoSchool'));
    ...
    private function getCommonLandingData()
    {
        $faqs = Faq::where('school_id', null)->get();
        $guidances = $this->guidance->builder()->get();
        $languages = Language::get();
        ...
        return compact('faqs', 'guidances', 'languages', 'settings', 'trail_package', 'extraFields');
    }

    public function pricingPage()
    {
        return view('landing-pages.pricing', $this->getCommonLandingData());
    }
    public function refundPolicyPage()
    {
        return view('landing-pages.refund-policy', $this->getCommonLandingData());
    }
    public function privacyPolicyPage()
    {
        return view('landing-pages.privacy-policy', $this->getCommonLandingData());
    }
    public function dataSecurityPage()
    {
        return view('landing-pages.data-security', $this->getCommonLandingData());
    }
    public function cookiesPolicyPage()
    {
        return view('landing-pages.cookies-policy', $this->getCommonLandingData());
    }
