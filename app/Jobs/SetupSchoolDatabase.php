...
final class SetupSchoolDatabase implements ShouldQueue
{
    ...
    public function handle(
        ...
        try {
            ...
            DB::setDefaultConnection('mysql');
            $school = School::on('mysql')->with('user')->findOrFail($this->schoolId);
            $settings = $cache->getSystemSettings();
            ...
            Mail::send('schools.email', $data, static function ($message) use ($data, $settings) {
                $fromEmail = !empty($settings['mail_send_from']) ? $settings['mail_send_from'] : (!empty($settings['mail_username']) ? $settings['mail_username'] : config('mail.from.address', 'hello@example.com'));
